...
지도 학습을 연상하는 하나의 방법은 우리가 데이터셋에 정의된 함수 *f* 를 만드는 것이라고 이미지화 하는 것이다. 우리의 data set은 *feature:특징*으로 구성된 정보를 구성하고 있을 것이다. 머신 러닝 알고리즘은 값을 살펴보고 실수값에 대해서는 회귀를, 혹은 이산적인 분류 집합의 범위로 값을 사상할 것이다. 이 알고리즘은 라벨링된 데이터에 가장 적합한 함수를 정의할 때까지 많은 예제(라벨링되어진 데이터)를 수행할 것이다. 이 함수는 다음과 같이 정의되어진다.
