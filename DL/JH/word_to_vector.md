...
전체 텍스트가 너무 길어 `BPTT (back-propagation through time)`가있는 네트워크를 학습 할 수 없으므로 `생략된 BPTT`라는 일괄 처리 변형을 사용한다.
...
생략된 BPTT로 이 네트워크를 교육할 것이므로 `배치간에 상태를 저장`해야 한다.
...
즉, 랜덤한 잘린(생략된) 정상 값으로 채워진 특정 모양의 한 텐서를 반환한다.  
